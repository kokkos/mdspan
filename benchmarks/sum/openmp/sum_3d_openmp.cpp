...
#pragma omp parallel default(none) shared(repeats, s)
    {
      for (int r = 0; r < repeats; ++r) {
        value_type sum = 0;
        for (index_type i = omp_get_thread_num(); i < s.extent(0);
...
MDSPAN_BENCHMARK_ALL_3D(BM_MDSpan_Sum_3D_OpenMP, right_, rmdspan, 200, 200,
                        200);

//================================================================================

template <class MDSpan, class... DynSizes>
void BM_MDSpan_Sum_3D_loop_OpenMP(benchmark::State& state, MDSpan,
                                  DynSizes... dyn) {
  using value_type = typename MDSpan::value_type;
  auto buffer      = std::make_unique<value_type[]>(
      MDSpan{nullptr, dyn...}.mapping().required_span_size());
...
MDSPAN_BENCHMARK_ALL_3D(BM_MDSpan_Sum_3D_loop_OpenMP, right_, rmdspan, 200, 200,
                        200);
MDSPAN_BENCHMARK_ALL_3D(BM_MDSpan_Sum_3D_loop_OpenMP, left_, lmdspan, 200, 200,
                        200);

//================================================================================

template <class T, class SizeX, class SizeY, class SizeZ>
void BM_Raw_Sum_3D_OpenMP(benchmark::State& state, T, SizeX x, SizeY y,
                          SizeZ z) {
  auto buffer = std::make_unique<T[]>(x * y * z);
  {
    // just for setup...
    auto wrapped = Kokkos::mdspan<T, Kokkos::dextents<index_type, 1>>{
        buffer.get(), x * y * z};
...
#pragma omp parallel default(none) shared(data, x, y, z, repeats)
    {
      for (int r = 0; r < repeats; ++r) {
        ...
        for (index_type i = omp_get_thread_num(); i < x;
             i += omp_get_num_threads()) {
