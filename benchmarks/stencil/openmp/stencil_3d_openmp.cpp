...
void BM_MDSpan_OpenMP_Stencil_3D(benchmark::State& state, MDSpan,
                                 DynSizes... dyn) {
...
#pragma omp parallel for
  ...
  size_t num_inner_elements =
      (s.extent(0) - d) * (s.extent(1) - d) * (s.extent(2) - d);
  size_t stencil_num = (2 * d + 1) * (2 * d + 1) * (2 * d + 1);
  state.SetBytesProcessed(num_inner_elements * stencil_num *
                          sizeof(value_type) * state.iterations());
}
MDSPAN_BENCHMARK_ALL_3D(BM_MDSpan_OpenMP_Stencil_3D, right_, rmdspan, 80, 80,
                        80);
MDSPAN_BENCHMARK_ALL_3D(BM_MDSpan_OpenMP_Stencil_3D, left_, lmdspan, 80, 80,
                        80);
MDSPAN_BENCHMARK_ALL_3D(BM_MDSpan_OpenMP_Stencil_3D, right_, rmdspan, 400, 400,
                        400);
MDSPAN_BENCHMARK_ALL_3D(BM_MDSpan_OpenMP_Stencil_3D, left_, lmdspan, 400, 400,
                        400);

//================================================================================

template <class T, class SizeX, class SizeY, class SizeZ>
void BM_Raw_OpenMP_Stencil_3D_right(benchmark::State& state, T, SizeX x,
                                    SizeY y, SizeZ z) {
...
BENCHMARK_CAPTURE(BM_Raw_OpenMP_Stencil_3D_right, size_80_80_80, int(), 80, 80,
                  80);
BENCHMARK_CAPTURE(BM_Raw_OpenMP_Stencil_3D_right, size_400_400_400, int(), 400,
                  400, 400);

//================================================================================

template <class T, class SizeX, class SizeY, class SizeZ>
void BM_Raw_OpenMP_Stencil_3D_left(benchmark::State& state, T, SizeX x, SizeY y,
                                   SizeZ z) {
  using MDSpan     = Kokkos::mdspan<T, Kokkos::dextents<index_type, 3>>;
  using value_type = typename MDSpan::value_type;
  auto buffer_size = MDSpan{nullptr, x, y, z}.mapping().required_span_size();
...
BENCHMARK_CAPTURE(BM_Raw_OpenMP_Stencil_3D_left, size_80_80_80, int(), 80, 80,
                  80);
BENCHMARK_CAPTURE(BM_Raw_OpenMP_Stencil_3D_left, size_400_400_400, int(), 400,
                  400, 400);
...
template <class T, class SizeX, class SizeY, class SizeZ>
void BM_RawMDPtr_OpenMP_Stencil_3D_right(benchmark::State& state, T, SizeX x,
                                         SizeY y, SizeZ z) {
  using MDSpan     = Kokkos::mdspan<T, Kokkos::dextents<index_type, 3>>;
  using value_type = typename MDSpan::value_type;
  auto buffer_size = MDSpan{nullptr, x, y, z}.mapping().required_span_size();
...
BENCHMARK_CAPTURE(BM_RawMDPtr_OpenMP_Stencil_3D_right, size_80_80_80, int(), 80,
                  80, 80);
BENCHMARK_CAPTURE(BM_RawMDPtr_OpenMP_Stencil_3D_right, size_400_400_400, int(),
                  400, 400, 400);
//================================================================================
