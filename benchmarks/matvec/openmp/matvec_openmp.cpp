...
void BM_MDSpan_OpenMP_MatVec(benchmark::State& state, MDSpanMatrix,
                             DynSizes... dyn) {
  using value_type   = typename MDSpanMatrix::value_type;
  using MDSpanVector = lmdspan<value_type, Kokkos::dynamic_extent>;

  auto buffer_size_A =
      MDSpanMatrix{nullptr, dyn...}.mapping().required_span_size();
  ...
  auto buffer_size_x =
      MDSpanVector{nullptr, A.extent(1)}.mapping().required_span_size();
  ...
  auto buffer_size_y =
      MDSpanVector{nullptr, A.extent(0)}.mapping().required_span_size();
...
#pragma omp parallel for
  ...
  state.SetBytesProcessed(R * num_elements * sizeof(value_type) *
                          state.iterations() * global_repeat);
  state.counters["repeats"] = global_repeat;
}

BENCHMARK_CAPTURE(
    BM_MDSpan_OpenMP_MatVec, left,
    lmdspan<double, Kokkos::dynamic_extent, Kokkos::dynamic_extent>(), 100000,
    5000);
BENCHMARK_CAPTURE(
    BM_MDSpan_OpenMP_MatVec, right,
    rmdspan<double, Kokkos::dynamic_extent, Kokkos::dynamic_extent>(), 100000,
    5000);

template <class MDSpanMatrix, class... DynSizes>
void BM_MDSpan_OpenMP_MatVec_Raw_Left(benchmark::State& state, MDSpanMatrix,
                                      DynSizes... dyn) {
  using value_type   = typename MDSpanMatrix::value_type;
  using MDSpanVector = lmdspan<value_type, Kokkos::dynamic_extent>;
...
BENCHMARK_CAPTURE(
    BM_MDSpan_OpenMP_MatVec_Raw_Left, left,
    lmdspan<double, Kokkos::dynamic_extent, Kokkos::dynamic_extent>(), 100000,
    5000);

template <class MDSpanMatrix, class... DynSizes>
void BM_MDSpan_OpenMP_MatVec_Raw_Right(benchmark::State& state, MDSpanMatrix,
                                       DynSizes... dyn) {
  using value_type   = typename MDSpanMatrix::value_type;
  using MDSpanVector = lmdspan<value_type, Kokkos::dynamic_extent>;
...
BENCHMARK_CAPTURE(
    BM_MDSpan_OpenMP_MatVec_Raw_Right, right,
    rmdspan<double, Kokkos::dynamic_extent, Kokkos::dynamic_extent>(), 100000,
    5000);
