...
void BM_MDSpan_TinyMatrixSum_right(benchmark::State& state, MDSpan,
                                   DynSizes... dyn) {
...
MDSPAN_BENCHMARK_ALL_3D(BM_MDSpan_TinyMatrixSum_right, right_, lmdspan, 1000000,
                        3, 3);
MDSPAN_BENCHMARK_ALL_3D(BM_MDSpan_TinyMatrixSum_right, left_, lmdspan, 1000000,
                        3, 3);

//================================================================================

template <class T, class SizeX, class SizeY, class SizeZ>
void BM_Raw_Static_TinyMatrixSum_right(benchmark::State& state, T, SizeX x,
                                       SizeY y, SizeZ z) {
  ...
  state.SetBytesProcessed(num_inner_elements * 3 * sizeof(value_type) *
                          state.iterations());
}
BENCHMARK_CAPTURE(BM_Raw_Static_TinyMatrixSum_right, size_1000000_3_3, int(),
                  1000000, 3, 3);

//================================================================================

template <class T, class SizeX, class SizeY, class SizeZ>
void BM_Raw_TinyMatrixSum_right(benchmark::State& state, T, SizeX x, SizeY y,
                                SizeZ z) {
  ...
  size_t num_inner_elements = x * y * z;
  state.SetBytesProcessed(num_inner_elements * y * sizeof(value_type) *
                          state.iterations());
}
BENCHMARK_CAPTURE(BM_Raw_TinyMatrixSum_right, size_1000000_3_3, int(),
                  size_t(1000000), size_t(3), size_t(3));
