...
void BM_MDSpan_OpenMP_noloop_TinyMatrixSum(benchmark::State& state, MDSpan,
                                           DynSizes... dyn) {
...
#pragma omp parallel
  ...
  state.SetBytesProcessed(num_elements * 3 * sizeof(value_type) *
                          state.iterations() * global_repeat);
  state.counters["repeats"] = global_repeat;
}
MDSPAN_BENCHMARK_ALL_3D(BM_MDSpan_OpenMP_noloop_TinyMatrixSum, right_, rmdspan,
                        1000000, 3, 3);
MDSPAN_BENCHMARK_ALL_3D(BM_MDSpan_OpenMP_noloop_TinyMatrixSum, left_, lmdspan,
                        1000000, 3, 3);

//================================================================================

template <class MDSpan, class... DynSizes>
void BM_MDSpan_OpenMP_TinyMatrixSum(benchmark::State& state, MDSpan,
                                    DynSizes... dyn) {
  using value_type = typename MDSpan::value_type;
...
MDSPAN_BENCHMARK_ALL_3D_REAL_TIME(BM_MDSpan_OpenMP_TinyMatrixSum, right_,
                                  rmdspan, 1000000, 3, 3);
MDSPAN_BENCHMARK_ALL_3D_REAL_TIME(BM_MDSpan_OpenMP_TinyMatrixSum, left_,
                                  lmdspan, 1000000, 3, 3);

//================================================================================

template <class T, class SizeX, class SizeY, class SizeZ>
void BM_Raw_Static_OpenMP_TinyMatrixSum_right(benchmark::State& state, T,
...
#pragma omp parallel for
  ...
  state.SetBytesProcessed(num_inner_elements * 3 * global_repeat *
                          sizeof(value_type) * state.iterations());
  state.counters["repeats"] = global_repeat;
}
BENCHMARK_CAPTURE(BM_Raw_Static_OpenMP_TinyMatrixSum_right, size_1000000_3_3,
                  int(), 1000000, 3, 3)
    ->UseRealTime();

template <class T, class SizeX, class SizeY, class SizeZ>
void BM_Raw_OpenMP_TinyMatrixSum_right(benchmark::State& state, T, SizeX x,
                                       SizeY y, SizeZ z) {
  using MDSpan     = Kokkos::mdspan<T, Kokkos::dextents<index_type, 3>>;
  using value_type = typename MDSpan::value_type;
  auto buffer_size = MDSpan{nullptr, x, y, z}.mapping().required_span_size();
...
#pragma omp parallel for
  ...
  size_t num_inner_elements = x * y * z;
  state.SetBytesProcessed(num_inner_elements * y * global_repeat *
                          sizeof(value_type) * state.iterations());
  state.counters["repeats"] = global_repeat;
}
BENCHMARK_CAPTURE(BM_Raw_OpenMP_TinyMatrixSum_right, size_1000000_3_3, int(),
                  1000000, 3, 3)
    ->UseRealTime();
...
template <class T, class SizeX, class SizeY, class SizeZ>
void BM_Raw_Static_OpenMP_TinyMatrixSum_left(benchmark::State& state, T,
                                             SizeX x, SizeY y, SizeZ z) {
  using MDSpan =
      Kokkos::mdspan<T,
                     Kokkos::extents<index_type, Kokkos::dynamic_extent, 3, 3>>;
  using value_type = typename MDSpan::value_type;
...
BENCHMARK_CAPTURE(BM_Raw_Static_OpenMP_TinyMatrixSum_left, size_1000000_3_3,
                  int(), 1000000, 3, 3)
    ->UseRealTime();
...
template <class T, class SizeX, class SizeY, class SizeZ>
void BM_Raw_OpenMP_TinyMatrixSum_left(benchmark::State& state, T, SizeX x,
                                      SizeY y, SizeZ z) {
  using MDSpan =
      Kokkos::mdspan<T,
                     Kokkos::extents<index_type, Kokkos::dynamic_extent, 3, 3>>;
...
BENCHMARK_CAPTURE(BM_Raw_OpenMP_TinyMatrixSum_left, size_1000000_3_3, int(),
                  1000000, 3, 3)
    ->UseRealTime();
...
typename MDSpan::value_type*** make_3d_ptr_array(MDSpan s) {
  static_assert(
      std::is_same<typename MDSpan::layout_type, Kokkos::layout_right>::value,
  ...
  for (index_type i = 0; i < s.extent(0); i++) {
    ...
    for (index_type j = 0; j < s.extent(1); j++) ptr[i][j] = &s(i, j, 0);
  }
  return ptr;
}
...
void free_3d_ptr_array(T*** ptr, size_t extent_0) {
  for (size_t i = 0; i < extent_0; i++) delete[] ptr[i];
  delete[] ptr;
}

template <class T, class SizeX, class SizeY, class SizeZ>
void BM_RawMDPtr_OpenMP_TinyMatrixSum_right(benchmark::State& state, T, SizeX x,
                                            SizeY y, SizeZ z) {
  using MDSpan =
      Kokkos::mdspan<T,
                     Kokkos::extents<index_type, Kokkos::dynamic_extent, 3, 3>>;
...
BENCHMARK_CAPTURE(BM_RawMDPtr_OpenMP_TinyMatrixSum_right, size_1000000_3_3,
                  int(), 1000000, 3, 3)
    ->UseRealTime();
