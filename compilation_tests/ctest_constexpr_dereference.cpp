...
MDSPAN_STATIC_TEST(
    ...
    Kokkos::mdspan<double,
                   Kokkos::extents<size_t, simple_static_sum_test_1(-1)>>{
        nullptr}
        .extent(0) == 18);
#endif
...
constexpr int simple_test_1d_constexpr_in_type() {
  ...
  auto s     = Kokkos::mdspan<
      int, Kokkos::extents<size_t, simple_static_sum_test_1(-1)>>(data);
  // 4 + 14 + 18 + 1 = 37
...
constexpr bool stress_test_2d_single_element_stress_test_impl_2(
  ...
  using mdspan_t =
      Kokkos::mdspan<int, Kokkos::extents<size_t, Idx1, Idx2>, Layout>;
  using dyn_mdspan_1_t =
      Kokkos::mdspan<int, Kokkos::extents<size_t, Kokkos::dynamic_extent, Idx2>,
                     Layout>;
  using dyn_mdspan_2_t =
      Kokkos::mdspan<int, Kokkos::extents<size_t, Idx1, Kokkos::dynamic_extent>,
                     Layout>;
  using dyn_mdspan_t = Kokkos::mdspan<
      int,
      Kokkos::extents<size_t, Kokkos::dynamic_extent, Kokkos::dynamic_extent>,
  ...
  for (ptrdiff_t i = 0; i < Idx1; ++i) {
    for (ptrdiff_t j = 0; j < Idx2; ++j) {
      MDSPAN_IMPL_OP(s, i, j)      = MDSPAN_IMPL_OP(s1, i, j) =
          MDSPAN_IMPL_OP(s2, i, j) = MDSPAN_IMPL_OP(s12, i, j) = 1;
    }
  }
  int result = 0;
  for (ptrdiff_t i = 0; i < Idx1; ++i) {
    for (ptrdiff_t j = 0; j < Idx2; ++j) {
      result += MDSPAN_IMPL_OP(s, i, j) + MDSPAN_IMPL_OP(s1, i, j) +
                MDSPAN_IMPL_OP(s2, i, j) + MDSPAN_IMPL_OP(s12, i, j);
    }
  }
  result /= 4;
