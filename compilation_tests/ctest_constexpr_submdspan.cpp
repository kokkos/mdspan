...
constexpr bool dynamic_extent_1d_all_slice() {
  int data[] = {1, 2, 3, 4, 5};
  auto s = Kokkos::mdspan<int, Kokkos::extents<size_t, Kokkos::dynamic_extent>,
                          Layout>(data, 5);
  int result = 0;
...
constexpr bool dynamic_extent_1d_pair_full() {
  ...
  int result = 0;
  auto ss =
      Kokkos::submdspan(s, std::pair<std::ptrdiff_t, std::ptrdiff_t>{0, 5});
  for (size_t i = 0; i < s.extent(0); ++i) {
    result += MDSPAN_IMPL_OP(ss, i);
  }
  constexpr_assert_equal(15, result);
...
constexpr bool dynamic_extent_1d_pair_each() {
  ...
  int result = 0;
  for (size_t i = 0; i < s.extent(0); ++i) {
    auto ss = Kokkos::submdspan(
        s, std::pair<std::ptrdiff_t, std::ptrdiff_t>{i, i + 1});
...
MDSPAN_STATIC_TEST(
    // -1 - 2 - 3 + 7 + 8 + 9 = 18
    Kokkos::mdspan<double,
                   Kokkos::extents<size_t, simple_static_submdspan_test_1(-1)>>{
        nullptr}
        .extent(0) == 18);
#endif
...
constexpr bool mixed_submdspan_left_test_2() {
  ...
  auto s =
      Kokkos::mdspan<int, Kokkos::extents<size_t, 3, Kokkos::dynamic_extent>,
                     Kokkos::layout_left>(data, 5);
  int result = 0;
...
// MSVC ICE
#ifndef MDSPAN_IMPL_COMPILER_MSVC
MDSPAN_STATIC_TEST(mixed_submdspan_test_3<Kokkos::layout_right>());
#endif

//==============================================================================

#if defined(MDSPAN_ENABLE_EXPENSIVE_COMPILATION_TESTS) && \
    MDSPAN_ENABLE_EXPENSIVE_COMPILATION_TESTS
...
constexpr bool submdspan_single_element_stress_test_impl_2(
  ...
  using dyn_mdspan_t = Kokkos::mdspan<
      ...
      Kokkos::extents<size_t,
                      _repeated_ptrdiff_t<Kokkos::dynamic_extent, Idxs>...>,
      Layout>;
  int data[]  = {42};
  ...
  auto ss_all = Kokkos::submdspan(
      s, _repeated_with_idxs_t<Kokkos::full_extent_t, Idxs>{}...);
  auto ss_all_dyn = Kokkos::submdspan(
      s_dyn, _repeated_with_idxs_t<Kokkos::full_extent_t, Idxs>{}...);
  ...
  auto ss_pair = Kokkos::submdspan(
      s, _repeated_with_idxs_t<std::pair<ptrdiff_t, ptrdiff_t>, Idxs>{0, 1}...);
  auto ss_pair_dyn = Kokkos::submdspan(
      s_dyn,
      _repeated_with_idxs_t<std::pair<ptrdiff_t, ptrdiff_t>, Idxs>{0, 1}...);
  auto val_pair = MDSPAN_IMPL_OP(ss_pair, (_repeated_ptrdiff_t<0, Idxs>...));
  auto val_pair_dyn =
      MDSPAN_IMPL_OP(ss_pair_dyn, (_repeated_ptrdiff_t<0, Idxs>...));
  ...
  return MDSPAN_IMPL_OP0(ss) == 42 && MDSPAN_IMPL_OP0(ss_dyn) == 42 &&
         val == 42 && val_dyn == 42 && val_pair == 42 && val_pair_dyn == 42;
}

template <class Layout, size_t... Sizes>
constexpr bool submdspan_single_element_stress_test_impl_1(
