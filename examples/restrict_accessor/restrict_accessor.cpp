...
#define MDSPAN_IMPL_RESTRICT_POINTER(ELEMENT_TYPE) \
  ELEMENT_TYPE* MDSPAN_IMPL_RESTRICT_KEYWORD
...
// }
//
// This is relevant because restrict_accessor<ElementType>::reference is _not_
// restrict. (It's not formally correct to apply C restrict wording to C++
// references. However, GCC defines this extension:
//
// https://gcc.gnu.org/onlinedocs/gcc/Restricted-Pointers.html
//
// In what follows, I'll assume that this has a reasonable definition.)
// The idea is that even though p[i] has type ElementType& and not ElementType&
// restrict, the compiler can figure out that the reference comes from a pointer
// based on p, which is marked restrict.
//
...
struct restrict_accessor {
  ...
  MDSPAN_TEMPLATE_REQUIRES(
      ...
      /* requires */ (std::is_convertible<OtherElementType (*)[],
                                          element_type (*)[]>::value))
...
using restrict_mdspan_1d =
    Kokkos::mdspan<ElementType, Kokkos::dextents<index_type, 1>,
                   Kokkos::layout_right, restrict_accessor<ElementType>>;
...
template <class ElementType>
void add_restrict_mdspan_1d(restrict_mdspan_1d<const ElementType> x,
                            restrict_mdspan_1d<const ElementType> y,
                            restrict_mdspan_1d<ElementType> z) {
...
template <class ElementType>
auto benchmark_add_restrict_mdspan_1d(const std::size_t num_trials,
...
template <class ElementType>
auto benchmark_add_restrict_raw_1d(const std::size_t num_trials,
                                   const index_type n, const ElementType* x,
                                   const ElementType* y, ElementType* z) {
  TICK();
...
template <class ElementType>
auto benchmark_add_unrestrict_raw_1d(const std::size_t num_trials,
                                     const index_type n, const ElementType* x,
                                     const ElementType* y, ElementType* z) {
  TICK();
  MDSPAN_IMPL_RESTRICT_POINTER(const ElementType) x2 = x;
...
template <class ElementType>
auto warmup(const std::size_t num_trials, const index_type n,
            const ElementType* x, const ElementType* y, ElementType* z) {
...
int main(int argc, char* argv[]) {
  ...
  auto restrict_mdspan_result = benchmark_add_restrict_mdspan_1d(
      num_trials, n, x.data(), y.data(), z.data());
  auto mdspan_result =
      benchmark_add_mdspan_1d(num_trials, n, x.data(), y.data(), z.data());
  auto raw_result =
      benchmark_add_raw_1d(num_trials, n, x.data(), y.data(), z.data());
  auto restrict_raw_result = benchmark_add_restrict_raw_1d(
      num_trials, n, x.data(), y.data(), z.data());
  auto unrestrict_raw_result = benchmark_add_unrestrict_raw_1d(
      num_trials, n, x.data(), y.data(), z.data());
