...
constexpr std::size_t min_byte_alignment =
    min_overalignment_factor * sizeof(float);

// Use int, not size_t, as the index_type.
// Some compilers have trouble optimizing loops with unsigned or 64-bit index
// types.
...
#define MDSPAN_IMPL_ASSUME_ALIGNED(ELEMENT_TYPE, POINTER, BYTE_ALIGNMENT) \
...
#define MDSPAN_IMPL_ASSUME_ALIGNED(ELEMENT_TYPE, POINTER, BYTE_ALIGNMENT) \
  reinterpret_cast<ELEMENT_TYPE*>(                                        \
      __builtin_assume_aligned(POINTER, BYTE_ALIGNMENT))
...
constexpr char align_attribute_method[] =
    "__declspec(align_value(BYTE_ALIGNMENT))";
#elif defined(__ICC)
#define MDSPAN_IMPL_ALIGN_VALUE_ATTRIBUTE(BYTE_ALIGNMENT) \
...
constexpr bool valid_byte_alignment(const std::size_t byte_alignment) {
  return is_nonzero_power_of_two(byte_alignment) &&
         byte_alignment >= alignof(ElementType);
}
...
#if defined(__ICC)
  // x86-64 ICC 2021.5.0 emits warning #3186 ("expected typedef declaration")
  // here. No other compiler (including Clang, which has a similar type
  // attribute) has this issue.
#pragma warning push
...
template <class ElementType, std::size_t byte_alignment>
using aligned_pointer_t =
    typename aligned_pointer<ElementType, byte_alignment>::type;
...
struct aligned_accessor {
  ...
  MDSPAN_TEMPLATE_REQUIRES(
      class OtherElementType, std::size_t other_byte_alignment,
      /* requires */
...
template <class ElementType>
using allocation_t = std::unique_ptr<ElementType[], delete_raw<ElementType>>;

template <class ElementType, std::size_t byte_alignment>
allocation_t<ElementType> allocate_raw(const std::size_t num_elements) {
  static_assert(byte_alignment >= sizeof(ElementType),
                "byte_alignment must be at least sizeof(ElementType).");
  static constexpr std::size_t overalignment =
      byte_alignment / sizeof(ElementType);
  static_assert(
      overalignment * sizeof(ElementType) == byte_alignment,
  ...
  // aligned_alloc is a C11 function.  Apple Clang and GCC do not provide it
  // with -std=c++14.  We have coverage for the Windows case (at least with
  // MSVC) above, so we can resort to the POSIX function posix_memalign.
  const int err = posix_memalign(&ptr, byte_alignment, num_bytes);
  if (err != 0) {
    if (err == EINVAL) {
      throw std::runtime_error(
          "posix_memalign failed: alignment not a power of two");
    } else if (err == ENOMEM) {
...
class aligned_array_allocation {
 ...
 public:
  aligned_array_allocation(std::size_t number_of_elements)
      : allocation(
            allocate_raw<ElementType, byte_alignment>(number_of_elements)),
...
class deliberately_unaligned_array_allocation {
 public:
  deliberately_unaligned_array_allocation(std::size_t number_of_elements)
      : allocation(allocate_raw<ElementType, sizeof(ElementType)>(
            number_of_elements + 1)),
        pointer(unaligned_pointer(allocation.get())),
        num_elements(number_of_elements) {}

  ElementType* data() const { return pointer; }
 ...
 private:
  ...
  static ElementType* unaligned_pointer(ElementType* allocation_pointer) {
    if (allocation_pointer == nullptr) {
      return nullptr;
    }
    const auto ptr_as_uint =
        reinterpret_cast<std::uintptr_t>(allocation_pointer);
...
using aligned_mdspan_1d =
    Kokkos::mdspan<ElementType, Kokkos::dextents<index_type, 1>,
                   Kokkos::layout_right,
...
using mdspan_1d =
    Kokkos::mdspan<ElementType, Kokkos::dextents<index_type, 1>,
                   Kokkos::layout_right, Kokkos::default_accessor<ElementType>>;
...
#define TOCK()                                 \
  std::chrono::duration_cast<double_seconds>(  \
      std::chrono::steady_clock::now() - tick) \
      .count()

template <class ElementType, std::size_t byte_alignment>
void add_aligned_mdspan_1d(
    aligned_mdspan_1d<const ElementType, byte_alignment> x,
...
template <class ElementType, std::size_t byte_alignment>
auto benchmark_add_aligned_mdspan_1d(
    const std::size_t num_trials, const index_type n, const ElementType x[],
    const ElementType y[], ElementType z[],
    std::integral_constant<std::size_t, byte_alignment> /* ba */) {
  TICK();
...
void add_mdspan_1d(mdspan_1d<const ElementType> x,
                   mdspan_1d<const ElementType> y, mdspan_1d<ElementType> z) {
  const index_type n = z.extent(0);
  for (index_type i = 0; i < n; ++i) {
    z[i] = x[i] + y[i];
  }
...
template <class ElementType>
auto benchmark_add_mdspan_1d(const std::size_t num_trials, const index_type n,
                             const ElementType x[], const ElementType y[],
                             ElementType z[]) {
  TICK();
...
template <class ElementType>
void add_raw_1d(const index_type n, const ElementType x[],
                const ElementType y[], ElementType z[]) {
  for (index_type i = 0; i < n; ++i) {
    z[i] = x[i] + y[i];
  }
...
template <class ElementType>
auto benchmark_add_raw_1d(const std::size_t num_trials, const index_type n,
                          const ElementType x[], const ElementType y[],
                          ElementType z[]) {
  TICK();
  for (std::size_t trial = 0; trial < num_trials; ++trial) {
...
auto benchmark_add_aligned_raw_1d(
  ...
  for (std::size_t trial = 0; trial < num_trials; ++trial) {
    add_aligned_raw_1d<ElementType, byte_alignment>(n, x_blessed, y_blessed,
                                                    z_blessed);
  }
  return TOCK();
}
...
#ifdef _OPENMP
template <class ElementType, std::size_t byte_alignment>
void add_omp_simd_aligned_mdspan_1d(
    aligned_mdspan_1d<const ElementType, byte_alignment> x,
    aligned_mdspan_1d<const ElementType, byte_alignment> y,
    aligned_mdspan_1d<ElementType, byte_alignment> z) {
  const index_type n = z.extent(0);
...
template <class ElementType, std::size_t byte_alignment>
auto benchmark_add_omp_simd_aligned_mdspan_1d(
    const std::size_t num_trials, const index_type n, const ElementType x[],
    const ElementType y[], ElementType z[],
    std::integral_constant<std::size_t, byte_alignment> /* ba */) {
  TICK();
...
auto benchmark_add_omp_simd_mdspan_1d(const std::size_t num_trials,
                                      const index_type n, const ElementType x[],
                                      const ElementType y[], ElementType z[]) {
  TICK();
  mdspan_1d<const ElementType> x2{x, n};
  mdspan_1d<const ElementType> y2{y, n};
...
template <class ElementType>
void add_omp_simd_raw_1d(const index_type n, const ElementType x[],
                         const ElementType y[], ElementType z[]) {
#pragma omp simd
  for (index_type i = 0; i < n; ++i) {
    z[i] = x[i] + y[i];
...
template <class ElementType, std::size_t byte_alignment>
void add_omp_aligned_simd_raw_1d(
    const index_type n, const ElementType x[], const ElementType y[],
    ElementType z[],
    std::integral_constant<std::size_t, byte_alignment> /* ba */) {
...
template <class ElementType, std::size_t byte_alignment>
void add_omp_simd_aligned_raw_1d(
    const index_type n, aligned_pointer_t<const ElementType, byte_alignment> x,
    aligned_pointer_t<const ElementType, byte_alignment> y,
    aligned_pointer_t<ElementType, byte_alignment> z) {
#pragma omp simd
  for (index_type i = 0; i < n; ++i) {
...
auto benchmark_add_omp_simd_aligned_raw_1d(
  ...
  auto z_blessed = bless(z, ba);
  for (std::size_t trial = 0; trial < num_trials; ++trial) {
    add_omp_simd_aligned_raw_1d<ElementType, byte_alignment>(
...
template <class ElementType, std::size_t byte_alignment>
void add_omp_aligned_simd_aligned_raw_1d(
    const index_type n, aligned_pointer_t<const ElementType, byte_alignment> x,
    aligned_pointer_t<const ElementType, byte_alignment> y,
    aligned_pointer_t<ElementType, byte_alignment> z) {
#pragma omp simd aligned(z, x, y : byte_alignment)
...
auto benchmark_add_omp_aligned_simd_aligned_raw_1d(
  ...
  for (std::size_t trial = 0; trial < num_trials; ++trial) {
    ...
    add_omp_aligned_simd_aligned_raw_1d<ElementType, byte_alignment>(
        n, x_blessed, y_blessed, z_blessed);
  }
  return TOCK();
}
...
template <class ElementType>
void set_elements_of_arrays(const index_type n, ElementType x[],
                            ElementType y[], ElementType z[]) {
  for (index_type i = 0; i < n; ++i) {
    x[i] = 1.0;
    y[i] = 2.0;
    z[i] = 0.0;
...
int main(int argc, char* argv[]) {
  ...
  using std::cout;
  using std::endl;
  constexpr std::integral_constant<std::size_t, min_byte_alignment>
      byte_alignment;
  ...
  set_elements_of_arrays(n, x_aligned.data(), y_aligned.data(),
                         z_aligned.data());

  auto aligned_mdspan_result = benchmark_add_aligned_mdspan_1d(
      num_trials, n, x_aligned.data(), y_aligned.data(), z_aligned.data(),
      byte_alignment);
  auto aligned_raw_result = benchmark_add_aligned_raw_1d(
      num_trials, n, x_aligned.data(), y_aligned.data(), z_aligned.data(),
      byte_alignment);
#ifdef _OPENMP
  auto omp_simd_aligned_mdspan_result =
      benchmark_add_omp_simd_aligned_mdspan_1d(
          num_trials, n, x_aligned.data(), y_aligned.data(), z_aligned.data(),
          byte_alignment);
  auto omp_aligned_simd_raw_result = benchmark_add_omp_aligned_simd_raw_1d(
      num_trials, n, x_aligned.data(), y_aligned.data(), z_aligned.data(),
      byte_alignment);
  auto omp_simd_aligned_raw_result = benchmark_add_omp_simd_aligned_raw_1d(
      num_trials, n, x_aligned.data(), y_aligned.data(), z_aligned.data(),
      byte_alignment);
  auto omp_aligned_simd_aligned_raw_result =
      benchmark_add_omp_aligned_simd_aligned_raw_1d(
          num_trials, n, x_aligned.data(), y_aligned.data(), z_aligned.data(),
          byte_alignment);
#endif  // _OPENMP
  ...
  set_elements_of_arrays(n, x_unaligned.data(), y_unaligned.data(),
                         z_unaligned.data());
  ...
  cout << "Total time in seconds for OpenMP (omp simd) loops:" << endl
       << "  omp_simd_aligned_mdspan: " << omp_simd_aligned_mdspan_result
       << endl
