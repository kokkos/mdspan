...
int main() {
  ...
  using tiled_layout_type =
      typename SimpleTileLayout2D::template mapping<extents_type>;
  using row_major_mdspan =
      Kokkos::mdspan<int, extents_type, Kokkos::layout_right>;
  //----------------------------------------
  auto tiled = tiled_mdspan(
      data_tiled, tiled_layout_type(extents_type(n_rows, n_cols), 3, 3));
  ...
  for (int irow = 0; irow < n_rows; ++irow) {
    for (int icol = 0; icol < n_cols; ++icol) {
      ...
      if (tiled[irow, icol] != row_major[irow, icol]) {
        std::cout << "Mismatch for entry " << irow << ", " << icol << ":"
                  << std::endl;
