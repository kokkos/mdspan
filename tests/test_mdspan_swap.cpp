...
void test_mdspan_foo_swap_dynamic_extents() {
  ...
  dispatch([=] MDSPAN_IMPL_HOST_DEVICE() {
    using map_t =
        Foo::layout_foo::template mapping<Kokkos::dextents<size_t, 2>>;
    ...
    map_t map1(exts1);
    acc_t acc1(&flag1);
    Kokkos::mdspan<int, Kokkos::dextents<size_t, 2>, Foo::layout_foo, acc_t> m1(
        Foo::foo_ptr<int>{data1}, map1, acc1);
    Kokkos::dextents<size_t, 2> exts2(4, 3);
    map_t map2(exts2);
    acc_t acc2(&flag2);
    Kokkos::mdspan<int, Kokkos::dextents<size_t, 2>, Foo::layout_foo, acc_t> m2(
        Foo::foo_ptr<int>{data2}, map2, acc2);
