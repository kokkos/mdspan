...
struct foo_accessor {
  ...
  MDSPAN_INLINE_FUNCTION constexpr foo_accessor(
  ...
  template <class OtherElementType>
  MDSPAN_INLINE_FUNCTION constexpr foo_accessor(
      foo_accessor<OtherElementType> other) noexcept {
    flag = other.flag;
  }
...
class layout_foo::mapping {
 ...
 private:
  static_assert(Kokkos::detail::impl_is_extents_v<extents_type>,
                ...
                "specialization of Kokkos::extents.");
 ...
 public:
  ...
  MDSPAN_CONDITIONAL_EXPLICIT(
      (!std::is_convertible<OtherExtents,
                            extents_type>::value))  // needs two () due to comma
  MDSPAN_INLINE_FUNCTION MDSPAN_IMPL_CONSTEXPR_14
  mapping(Kokkos::layout_right::mapping<OtherExtents> const&
              other) noexcept  // NOLINT(google-explicit-constructor)
      : m_extents(other.extents()) {}
  ...
  MDSPAN_CONDITIONAL_EXPLICIT(
      (!std::is_convertible<OtherExtents,
                            extents_type>::value))  // needs two () due to comma
  MDSPAN_INLINE_FUNCTION MDSPAN_IMPL_CONSTEXPR_14
  mapping(Kokkos::layout_left::mapping<OtherExtents> const&
              other) noexcept  // NOLINT(google-explicit-constructor)
      : m_extents(other.extents()) {}
  ...
  MDSPAN_INLINE_FUNCTION MDSPAN_IMPL_CONSTEXPR_14
  mapping(Kokkos::layout_stride::mapping<OtherExtents> const&
 ...
 */
    ...
    for (rank_type r = m_extents.rank(); r > 0; r--) {
      if (stride != other.stride(r - 1))
        throw std::runtime_error(
            "Assigning layout_stride to layout_foo with invalid strides.");
  ...
  friend constexpr auto submdspan_mapping(const mapping& src,
    ...
    // i.e. we don't need to implement everything here, we just reuse
    // submdspan_mapping for layout_right
    ...
    if constexpr (std::is_same_v<
                      typename decltype(sub_right.mapping)::layout_type,
                      Kokkos::layout_right>) {
      // NVCC does not like deduction here, so get the extents type explicitly
      using sub_ext_t = std::remove_const_t<
          std::remove_reference_t<decltype(sub_right.mapping.extents())>>;
      auto sub_mapping =
          layout_foo::mapping<sub_ext_t>(sub_right.mapping.extents());
      return Kokkos::submdspan_mapping_result<decltype(sub_mapping)>{
          sub_mapping, sub_right.offset};
    } else {
      return sub_right;
    }
  }
