...
void test_submdspan_static_slice(
  ...
  std::vector<typename InputMdspan::value_type> storage(
  ...
  static_assert(
      std::is_same<typename output_mdspan_type::extents_type,
                   typename ExpectedOutputMdspan::extents_type>::value,
      "Extents don't match.");
  static_assert(std::is_same<typename output_mdspan_type::layout_type,
                             typename ExpectedOutputMdspan::layout_type>::value,
                "Layouts don't match.");
  static_assert(std::is_same<output_mdspan_type, ExpectedOutputMdspan>::value,
                "submdspan return types don't match.");
}
...
void test_submdspan_static_slice(
  ...
  test_submdspan_static_slice<ExpectedOutputMdspan, InputMdspan>(
      input_mapping, std::forward<Slices>(slices)...);
}

TEST(TestMdspan, StaticAsserts) {
  (void)test_info_;
  static_assert(std::is_convertible<IC<std::size_t, 1>, std::size_t>::value,
                "Just a check.");
  static_assert(std::is_convertible<IC<int32_t, 1>, std::size_t>::value,
                "Just a check.");
  static_assert(std::is_convertible<IC<uint32_t, 1>, std::size_t>::value,
                "Just a check.");
}
...
TEST(TestMdspan, SubmdspanStaticSlice_Left_iddd_FullFullIndex) {
  ...
  using input_mdspan_type =
      Kokkos::mdspan<float, input_extents_type, input_layout_type>;
...

  {
    ...
    using expected_output_mdspan_type =
        Kokkos::mdspan<float, expected_extents_type, expected_layout_type>;

    auto runTest = [&](auto integralConstant) {
      test_submdspan_static_slice<expected_output_mdspan_type,
                                  input_mdspan_type>(
          input_extents, Kokkos::full_extent, Kokkos::full_extent,
          integralConstant);
    };
    runTest(int(1));
...
TEST(TestMdspan, SubmdspanStaticSlice_Left_iddd_FullIndexFull) {
...

  {
    ...
    auto runTest = [&](auto integralConstant) {
      test_submdspan_static_slice<expected_output_mdspan_type,
                                  input_mdspan_type>(
          input_extents, Kokkos::full_extent, integralConstant,
          Kokkos::full_extent);
    };
    runTest(int(1));
    runTest(std::size_t(1));
...
TEST(TestMdspan, SubmdspanStaticSlice_Left_iddd_FullTupleFull) {
...

  {
    ...
    runTest(
        tuple<std::int64_t, std::int64_t>{std::int64_t(1), std::int64_t(3)});
    runTest(tuple<std::uint32_t, std::uint32_t>{std::uint32_t(1),
                                                std::uint32_t(3)});
  }
  {
    using expected_extents_type =
        Kokkos::extents<int, Kokkos::dynamic_extent, 2, Kokkos::dynamic_extent>;
    using expected_layout_type = Kokkos::layout_stride;
    using expected_output_mdspan_type =
        Kokkos::mdspan<float, expected_extents_type, expected_layout_type>;
    ...
    runTest(tuple<IC<std::size_t, 1>, IC<std::size_t, 3>>{
        IC<std::size_t, 1>{}, IC<std::size_t, 3>{}});
    runTest(tuple<IC<std::int64_t, 1>, IC<std::int64_t, 3>>{
        IC<std::int64_t, 1>{}, IC<std::int64_t, 3>{}});
    runTest(tuple<IC<std::uint32_t, 1>, IC<std::uint32_t, 3>>{
        IC<std::uint32_t, 1>{}, IC<std::uint32_t, 3>{}});
  }
}
...
TEST(TestMdspan, SubmdspanStaticSlice_Left_i345_FullTupleFull) {
  ...
  {
    using expected_extents_type =
        Kokkos::extents<int, 3, Kokkos::dynamic_extent, 5>;
    using expected_layout_type = Kokkos::layout_stride;
    using expected_output_mdspan_type =
        Kokkos::mdspan<float, expected_extents_type, expected_layout_type>;
...
TEST(TestMdspan, SubmdspanStaticSlice_Left_iddd_TupleFullTuple) {
...

  {
    ...
    runTest(tuple<std::size_t, std::size_t>{std::size_t{1}, std::size_t{3}},
            tuple<std::size_t, std::size_t>{std::size_t{1}, std::size_t{4}});
    runTest(
        tuple<std::int64_t, std::int64_t>{std::int64_t{1}, std::int64_t{3}},
        tuple<std::int64_t, std::int64_t>{std::int64_t{1}, std::int64_t{4}});
    runTest(
        tuple<std::uint32_t, std::uint32_t>{std::uint32_t{1}, std::uint32_t{3}},
        tuple<std::uint32_t, std::uint32_t>{std::uint32_t{1},
                                            std::uint32_t{4}});
  }
  {
    using expected_extents_type =
        Kokkos::extents<int, 2, Kokkos::dynamic_extent, 3>;
    using expected_layout_type =
        Kokkos::Experimental::layout_left_padded<Kokkos::dynamic_extent>;
    using expected_output_mdspan_type =
        Kokkos::mdspan<float, expected_extents_type, expected_layout_type>;
    ...
    runTest(tuple<IC<std::size_t, 1>, IC<std::size_t, 3>>{IC<std::size_t, 1>{},
                                                          IC<std::size_t, 3>{}},
            tuple<IC<std::size_t, 1>, IC<std::size_t, 4>>{
                IC<std::size_t, 1>{}, IC<std::size_t, 4>{}});
    runTest(
        tuple<IC<std::int64_t, 1>, IC<std::int64_t, 3>>{IC<std::int64_t, 1>{},
                                                        IC<std::int64_t, 3>{}},
        tuple<IC<std::int64_t, 1>, IC<std::int64_t, 4>>{IC<std::int64_t, 1>{},
                                                        IC<std::int64_t, 4>{}});
    runTest(
        tuple<IC<std::uint32_t, 1>, IC<std::uint32_t, 3>>{
            IC<std::uint32_t, 1>{}, IC<std::uint32_t, 3>{}},
        tuple<IC<std::uint32_t, 1>, IC<std::uint32_t, 4>>{
            IC<std::uint32_t, 1>{}, IC<std::uint32_t, 4>{}});
  }
}
...
TEST(TestMdspan, SubmdspanStaticSlice_Left_i345_TupleFullTuple) {
  ...
  {
    using expected_extents_type =
        Kokkos::extents<int, Kokkos::dynamic_extent, 4, Kokkos::dynamic_extent>;
...
TEST(TestMdspan, SubmdspanStaticSlice_Left_idd_FullTuple) {
  ...
  static_assert(
      std::is_convertible<std::tuple<IC<std::size_t, 1>, IC<std::size_t, 3>>,
  ...
  input_extents_type input_extents{3, 4};
  {
    ...
    auto runTest = [&](auto sliceSpec) {
      test_submdspan_static_slice<expected_output_mdspan_type,
                                  input_mdspan_type>(
          input_extents, Kokkos::full_extent, sliceSpec);
    };
    runTest(std::tuple<int, int>{int(1), int(3)});
    runTest(
        std::tuple<std::size_t, std::size_t>{std::size_t(1), std::size_t(3)});
    runTest(std::tuple<std::uint32_t, std::uint32_t>{std::uint32_t(1),
                                                     std::uint32_t(3)});
    runTest(std::tuple<std::int64_t, std::int64_t>{std::int64_t(1),
                                                   std::int64_t(3)});
  }
  {
    using expected_extents_type =
        Kokkos::extents<int, Kokkos::dynamic_extent, 2>;
    using expected_layout_type = Kokkos::layout_left;
    using expected_output_mdspan_type =
        Kokkos::mdspan<float, expected_extents_type, expected_layout_type>;
...
TEST(TestMdspan, SubmdspanStaticSlice_Left_idd_TupleFull) {
  ...
  input_extents_type input_extents{3, 4};
  {
    ...
    auto runTest = [&](auto sliceSpec) {
      test_submdspan_static_slice<expected_output_mdspan_type,
                                  input_mdspan_type>(input_extents, sliceSpec,
                                                     Kokkos::full_extent);
    };
    runTest(std::tuple<int, int>{int(1), int(3)});
    runTest(
  ...
  {
    using expected_extents_type =
        Kokkos::extents<int, 2, Kokkos::dynamic_extent>;
    using expected_layout_type =
        Kokkos::Experimental::layout_left_padded<Kokkos::dynamic_extent>;
    using expected_output_mdspan_type =
        Kokkos::mdspan<float, expected_extents_type, expected_layout_type>;
...
TEST(TestMdspan, SubmdspanStaticSlice_Stride_idd_TupleFull) {
  ...
  input_mapping_type input_mapping(input_extents, input_strides);
  {
    ...
    auto runTest = [&](auto sliceSpec) {
      test_submdspan_static_slice<expected_output_mdspan_type,
                                  input_mdspan_type>(input_mapping, sliceSpec,
                                                     Kokkos::full_extent);
    };
    runTest(std::tuple<int, int>{int(1), int(3)});
    runTest(
