...
template <size_t... Extents, size_t... DynamicSizes, size_t... StaticStrides,
...
struct TestLayoutStride<
  ...
  using mapping_type =
      typename Kokkos::layout_stride::template mapping<extents_type>;
  mapping_type map = {
      extents_type{DynamicSizes...},
...
using zero_stride_maps =
    ::testing::Types<layout_stride_case_t<extents_345_t, ints<>,
                                          ints<dyn, dyn, dyn>, ints<0, 0, 0>>,
                     layout_stride_case_t<extents_3dyn5_t, ints<4>,
                                          ints<dyn, dyn, dyn>, ints<0, 0, 0>>,
                     layout_stride_case_t<extents_ddd_t, ints<3, 4, 5>,
                                          ints<dyn, dyn, dyn>, ints<0, 0, 0>>>;
...
TEST(TestLayoutStrideSpanConstruction, test_from_span_construction) {
  using map_t =
      Kokkos::layout_stride::mapping<Kokkos::extents<size_t, dyn, dyn>>;
  std::array<int, 2> strides{1, 128};
  map_t m1(Kokkos::dextents<size_t, 2>{16, 32},
           std::span<int, 2>{strides.data(), 2});
  // The following is actually not supported by layout_stride in the standard?
  // map_t m2(Kokkos::dextents<size_t,2>{16, 32},
  // std::span<int,std::dynamic_extent>{strides.data(),2}); ASSERT_EQ(m1, m2);
...
TEST(TestLayoutStrideListInitialization, test_list_initialization) {
  Kokkos::layout_stride::mapping<Kokkos::extents<size_t, dyn, dyn>> m{
      Kokkos::dextents<size_t, 2>{16, 32}, std::array<int, 2>{1, 128}};
...
template <class>
struct TestLayoutEquality;
template <class Mapping, size_t... DynamicSizes, class Mapping2,
          size_t... DynamicSizes2, class Equality>
struct TestLayoutEquality<std::tuple<
    Mapping, std::integer_sequence<size_t, DynamicSizes...>, Mapping2,
    std::integer_sequence<size_t, DynamicSizes2...>, Equality>>
    : public ::testing::Test {
  using mapping_type1 = Mapping;
  using mapping_type2 = Mapping2;
  using extents_type1 = std::remove_reference_t<
      decltype(std::declval<mapping_type1>().extents())>;
  using extents_type2 = std::remove_reference_t<
      decltype(std::declval<mapping_type2>().extents())>;
  mapping_type1 map1 = {
      extents_type1{},
      std::array<size_t, sizeof...(DynamicSizes)>{DynamicSizes...}};
  mapping_type2 map2 = {
      extents_type2{},
      std::array<size_t, sizeof...(DynamicSizes2)>{DynamicSizes2...}};
  bool equal = Equality::value;
};

template <class E1, class S1, class E2, class S2, class Equal>
using test_stride_equality =
    std::tuple<typename Kokkos::layout_stride::template mapping<E1>, S1,
               typename Kokkos::layout_stride::template mapping<E2>, S2, Equal>;
template <size_t... Ds>
...
using equality_test_types =
    ::testing::Types<test_case_type<exts<16, 32>, sizes<1, 16>, exts<16, 32>,
                                    sizes<1, 16>, std::true_type>,
                     test_case_type<exts<16, 32>, sizes<1, 16>, exts<16, 32>,
                                    sizes<1, 17>, std::false_type>,
                     test_case_type<exts<16, 32>, sizes<1, 16>, exts<16, 64>,
                                    sizes<1, 16>, std::false_type>,
                     test_case_type<exts<16, 32>, sizes<1, 16>, exts<16, 64>,
                                    sizes<1, 17>, std::false_type>>;

using layout_stride_equality_test_types =
    equality_test_types<test_stride_equality>;
...
TEST(TestLayoutStrideCTAD, test_ctad) {
  ...
  /*
  Kokkos::layout_stride::mapping m0{Kokkos::extents{16, 32}, Kokkos::extents{1,
  128}}; ASSERT_EQ(m0.extents().rank(), 2);
  ...
  */
  Kokkos::layout_stride::mapping m1{Kokkos::extents{16, 32},
                                    std::array{1, 128}};
  ASSERT_EQ(m1.extents().rank(), 2);
  ASSERT_EQ(m1.extents().rank_dynamic(), 2);
  ASSERT_EQ(m1.extents().extent(0), 16);
  ...
  // TODO These won't work with our current implementation, because the array
  // will be deduced as the extent type, leading to a `static_assert`. We can
  // probably get around this with a clever refactoring, or by using an alias
  // template for `mapping` and deduction guides.
  /*
    Kokkos::layout_stride::mapping m2{std::array{16, 32}, Kokkos::extents{1,
    128}}; ASSERT_EQ(m2.extents().rank(), 2);
...
struct is_stride_avail<
    T, RankType,
    std::enable_if_t<std::is_same<decltype(std::declval<T>().stride(
                                      std::declval<RankType>())),
