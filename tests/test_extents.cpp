...
// can't create CUDA lambdas in the primary test functions, since they are
// private making a non-member function would require replicating all the type
// info

template <class>
struct TestExtents;
template <size_t... Extents, size_t... DynamicSizes>
struct TestExtents<std::tuple<Kokkos::extents<size_t, Extents...>,
  ...
  // Otherwise: "error: array must be initialized with a brace-enclosed
  // initializer"
  const std::array<size_t, sizeof...(Extents)> static_sizes{{Extents...}};
  const std::array<size_t, sizeof...(DynamicSizes)> dyn_sizes{
      {DynamicSizes...}};
  extents_type exts{DynamicSizes...};
  using Fixture =
      TestExtents<std::tuple<Kokkos::extents<size_t, Extents...>,
                             std::integer_sequence<size_t, DynamicSizes...>>>;
  ...
  void test_rank() {
    ...
    dispatch([=] MDSPAN_IMPL_HOST_DEVICE() {
      ...
      size_t dyn_val =
          exts.rank() > 0 ? static_cast<size_t>(exts.extent(0)) : 1;
      result[0] = dyn_val > 1e9 ? dyn_val : exts.rank();
      result[1] = exts.rank_dynamic();
      // Some compilers warn about unused exts since the functions are all
      // static constexpr
      (void)exts;
    });
  ...
  void test_extent() {
    ...
    dispatch([=] MDSPAN_IMPL_HOST_DEVICE() {
      extents_type exts(DynamicSizes...);
      for (size_t r = 0; r < exts.rank(); r++) result[r] = exts.extent(r);
      // Some compilers warn about unused _exts since the functions are all
      // static constexpr
      (void)exts;
    });
    int dyn_count = 0;
...
using extents_test_types = ::testing::Types<
    ...
    std::tuple<exts<Kokkos::dynamic_extent, Kokkos::dynamic_extent>,
               sizes<10, 3>>>;

TYPED_TEST_SUITE(TestExtents, extents_test_types);

TYPED_TEST(TestExtents, rank){MDSPAN_IMPL_TESTS_RUN_TEST(this->test_rank())}

TYPED_TEST(TestExtents, static_extent){
    MDSPAN_IMPL_TESTS_RUN_TEST(this->test_static_extent())}

TYPED_TEST(TestExtents, extent){MDSPAN_IMPL_TESTS_RUN_TEST(this->test_extent())}
...
template <size_t... Extents, size_t... DynamicSizes, size_t... Extents2,
          size_t... DynamicSizes2, bool ImplicitExts1ToExts2,
          bool ImplicitExts2ToExts1>
struct TestExtentsCompatCtors<
    std::tuple<Kokkos::extents<size_t, Extents...>,
               std::integer_sequence<size_t, DynamicSizes...>,
  ...
  struct make_extents<Exts, Idx, true, Static> {
    ...
    static Exts construct(SizeTypes... sizes) {
      return make_extents<Exts, Idx - 1, true, Static>::construct(Idx * 5,
                                                                  sizes...);
    }
  };

  // Construct dynamic extents - opnly add a parameter if the current index is
  // not static
  template <class Exts, size_t Idx>
  struct make_extents<Exts, Idx, false, false> {
    template <class... SizeTypes>
    static Exts construct(SizeTypes... sizes) {
      return make_extents<Exts, Idx - 1, false,
                          (Idx > 1 ? Exts::static_extent(Idx - 2) !=
...
using compatible_extents_test_types = ::testing::Types<
    std::tuple<exts<Kokkos::dynamic_extent>, sizes<5>, exts<5>, sizes<>,
               BoolPairDeducer<false, true>>,
    std::tuple<exts<5>, sizes<>, exts<Kokkos::dynamic_extent>, sizes<5>,
               BoolPairDeducer<true, false>>,
    //--------------------
    std::tuple<exts<Kokkos::dynamic_extent, 10>, sizes<5>,
               exts<5, Kokkos::dynamic_extent>, sizes<10>,
               BoolPairDeducer<false, false>>,
    std::tuple<exts<Kokkos::dynamic_extent, Kokkos::dynamic_extent>,
               sizes<5, 10>, exts<5, Kokkos::dynamic_extent>, sizes<10>,
               BoolPairDeducer<false, true>>,
    std::tuple<exts<Kokkos::dynamic_extent, Kokkos::dynamic_extent>,
               sizes<5, 10>, exts<Kokkos::dynamic_extent, 10>, sizes<5>,
               BoolPairDeducer<false, true>>,
    std::tuple<exts<Kokkos::dynamic_extent, Kokkos::dynamic_extent>,
               sizes<5, 10>, exts<5, 10>, sizes<>,
               BoolPairDeducer<false, true>>,
    std::tuple<exts<5, 10>, sizes<>, exts<5, Kokkos::dynamic_extent>, sizes<10>,
               BoolPairDeducer<true, false>>,
    std::tuple<exts<5, 10>, sizes<>, exts<Kokkos::dynamic_extent, 10>, sizes<5>,
               BoolPairDeducer<true, false>>,
    //--------------------
    std::tuple<exts<Kokkos::dynamic_extent, Kokkos::dynamic_extent, 15>,
               sizes<5, 10>, exts<5, Kokkos::dynamic_extent, 15>, sizes<10>,
               BoolPairDeducer<false, true>>,
    std::tuple<exts<5, 10, 15>, sizes<>, exts<5, Kokkos::dynamic_extent, 15>,
               sizes<10>, BoolPairDeducer<true, false>>,
    std::tuple<exts<5, 10, 15>, sizes<>,
               exts<Kokkos::dynamic_extent, Kokkos::dynamic_extent,
                    Kokkos::dynamic_extent>,
               sizes<5, 10, 15>, BoolPairDeducer<true, false>>>;
...
TEST(TestExtentsCtorStdArrayConvertibleToSizeT,
     test_extents_ctor_std_array_convertible_to_size_t) {
...
TEST(TestExtentsCtorStdArrayConvertibleToSizeT,
     test_extents_ctor_std_span_convertible_to_size_t) {
  std::array<int, 2> i{2, 2};
...
TYPED_TEST(TestExtentsCompatCtors, construct_from_dynamic_sizes) {
  ...
  constexpr bool e1_last_ext_static =
      e1_t::rank() > 0
          ? e1_t::static_extent(e1_t::rank() - 1) != Kokkos::dynamic_extent
          : true;
  e1_t e1 = TestFixture::template make_extents<e1_t, e1_t::rank(), false,
                                               e1_last_ext_static>::construct();
  for (size_t r = 0; r < e1.rank(); r++) ASSERT_EQ(e1.extent(r), (r + 1) * 5);

  using e2_t = typename TestFixture::extents_type2;
  constexpr bool e2_last_ext_static =
      e2_t::rank() > 0
          ? e2_t::static_extent(e2_t::rank() - 1) != Kokkos::dynamic_extent
          : true;
  e2_t e2 = TestFixture::template make_extents<e2_t, e2_t::rank(), false,
                                               e2_last_ext_static>::construct();
  for (size_t r = 0; r < e2.rank(); r++) ASSERT_EQ(e2.extent(r), (r + 1) * 5);
}

TYPED_TEST(TestExtentsCompatCtors, construct_from_all_sizes) {
  using e1_t = typename TestFixture::extents_type1;
  e1_t e1    = TestFixture::template make_extents<e1_t, e1_t::rank(), true,
                                               true>::construct();
  for (size_t r = 0; r < e1.rank(); r++) ASSERT_EQ(e1.extent(r), (r + 1) * 5);

  using e2_t = typename TestFixture::extents_type2;
  e2_t e2    = TestFixture::template make_extents<e2_t, e1_t::rank(), true,
                                               true>::construct();
  for (size_t r = 0; r < e2.rank(); r++) ASSERT_EQ(e2.extent(r), (r + 1) * 5);
}
