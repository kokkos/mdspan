...
> : public ::testing::Test {
  ...
  void test_extent() {
    ...
    int dyn_count = 0;
    for(size_t r=0; r<extents_type::rank(); r++) {
      bool is_dynamic = static_sizes[r] == md::dynamic_extent;
...
using extents_test_types =
  ::testing::Types<
    ...
    std::tuple<_exts<md::dynamic_extent>, _sizes<10>>,
    std::tuple<_exts<10, 3>, _sizes<>>,
    std::tuple<_exts<md::dynamic_extent, 3>, _sizes<10>>,
    std::tuple<_exts<10, md::dynamic_extent>, _sizes<3>>,
    std::tuple<_exts<md::dynamic_extent, md::dynamic_extent>, _sizes<10, 3>>
  >;
...
TYPED_TEST(TestExtents, default_ctor) {
  ...
  for (size_t r = 0; r < e.rank(); ++r) {
    bool is_dynamic = (e.static_extent(r) == md::dynamic_extent);
...
struct TestExtentsCompatCtors<std::tuple<
  md::extents<size_t,Extents...>,
  std::integer_sequence<size_t, DynamicSizes...>,
  md::extents<size_t,Extents2...>,
  std::integer_sequence<size_t, DynamicSizes2...>,
  _BoolPairDeducer<ImplicitExts1ToExts2,ImplicitExts2ToExts1>
>> : public ::testing::Test {
  using extents_type1 = md::extents<size_t,Extents...>;
  using extents_type2 = md::extents<size_t,Extents2...>;
  ...
  struct make_extents<Exts,Idx,false,false>
  {
    template<class ... SizeTypes>
    static Exts construct(SizeTypes...sizes) {
      return make_extents<Exts, Idx-1,false,(Idx>1?Exts::static_extent(Idx-2)!=md::dynamic_extent:true)>::construct(Idx*5, sizes...);
    }
  };
  template<class Exts, size_t Idx>
  struct make_extents<Exts,Idx,false,true>
  {
    template<class ... SizeTypes>
    static Exts construct(SizeTypes...sizes) {
      return make_extents<Exts, Idx-1,false,(Idx>1?Exts::static_extent(Idx-2)!=md::dynamic_extent:true)>::construct(sizes...);
    }
  };
...
using compatible_extents_test_types =
  ::testing::Types<
    std::tuple<_exts<md::dynamic_extent>, _sizes<5>, _exts<5>, _sizes<>, _BoolPairDeducer<false,true>>,
    std::tuple<_exts<5>, _sizes<>, _exts<md::dynamic_extent>, _sizes<5>, _BoolPairDeducer<true,false>>,
    //--------------------
    std::tuple<_exts<md::dynamic_extent, 10>, _sizes<5>, _exts<5, md::dynamic_extent>, _sizes<10>,  _BoolPairDeducer<false, false>>,
    std::tuple<_exts<md::dynamic_extent, md::dynamic_extent>, _sizes<5, 10>, _exts<5, md::dynamic_extent>, _sizes<10>,  _BoolPairDeducer<false, true>>,
    std::tuple<_exts<md::dynamic_extent, md::dynamic_extent>, _sizes<5, 10>, _exts<md::dynamic_extent, 10>, _sizes<5>,  _BoolPairDeducer<false, true>>,
    std::tuple<_exts<md::dynamic_extent, md::dynamic_extent>, _sizes<5, 10>, _exts<5, 10>, _sizes<>,  _BoolPairDeducer<false, true>>,
    std::tuple<_exts<5, 10>, _sizes<>, _exts<5, md::dynamic_extent>, _sizes<10>,  _BoolPairDeducer<true, false>>,
    std::tuple<_exts<5, 10>, _sizes<>, _exts<md::dynamic_extent, 10>, _sizes<5>,  _BoolPairDeducer<true, false>>,
    //--------------------
    std::tuple<_exts<md::dynamic_extent, md::dynamic_extent, 15>, _sizes<5, 10>, _exts<5, md::dynamic_extent, 15>, _sizes<10>,  _BoolPairDeducer<false, true>>,
    std::tuple<_exts<5, 10, 15>, _sizes<>, _exts<5, md::dynamic_extent, 15>, _sizes<10>,  _BoolPairDeducer<true, false>>,
    std::tuple<_exts<5, 10, 15>, _sizes<>, _exts<md::dynamic_extent, md::dynamic_extent, md::dynamic_extent>, _sizes<5, 10, 15>,  _BoolPairDeducer<true, false>>
  >;
...
TYPED_TEST(TestExtentsCompatCtors, construct_from_dynamic_sizes) {
  ...
  constexpr bool e1_last_ext_static = e1_t::rank()>0?e1_t::static_extent(e1_t::rank()-1)!=md::dynamic_extent:true;
  ...
  constexpr bool e2_last_ext_static = e2_t::rank()>0?e2_t::static_extent(e2_t::rank()-1)!=md::dynamic_extent:true;
...
TYPED_TEST(TestExtentsCompatCtors, construct_from_dynamic_array) {
  ...
  for(size_t r=0; r<e1_t::rank(); r++) {
    if(e1_t::static_extent(r)==md::dynamic_extent) {
  ...
  for(size_t r=0; r<e2_t::rank(); r++) {
    if(e2_t::static_extent(r)==md::dynamic_extent) {
...
TEST(TestExtentsCTADStdArray, test_extents_ctad_std_array) {
  md::extents m0{std::array<size_t, 0>{}};
  ASSERT_EQ(m0.rank(), 0);
  ASSERT_EQ(m0.rank_dynamic(), 0);

  // TODO: `extents` should accept an array of any type convertible to `size_t`.
  md::extents m1{std::array{64UL}};
  ASSERT_EQ(m1.rank(), 1);
  ASSERT_EQ(m1.rank_dynamic(), 1);
  ASSERT_EQ(m1.extent(0), 64);

  // TODO: `extents` should accept an array of any type convertible to `size_t`.
  md::extents m2{std::array{64UL, 128UL}};
  ASSERT_EQ(m2.rank(), 2);
  ASSERT_EQ(m2.rank_dynamic(), 2);
  ASSERT_EQ(m2.extent(0), 64);
  ASSERT_EQ(m2.extent(1), 128);
  ...
  // TODO: `extents` should accept an array of any type convertible to `size_t`.
  md::extents m3{std::array{64UL, 128UL, 256UL}};
  ASSERT_EQ(m3.rank(), 3);
  ASSERT_EQ(m3.rank_dynamic(), 3);
  ASSERT_EQ(m3.extent(0), 64);
  ASSERT_EQ(m3.extent(1), 128);
