...
constexpr void test_mdspan_types(const H& handle, const M& map, const A& acc) {
  ...
  //   'checked_accessor<const double>' is deprecated because it has a
  //   user-provided copy constructor
  //   [-Werror,-Wdeprecated-copy-with-user-provided-copy]
  if constexpr (!std::is_same_v<A, checked_accessor<const double>>) m = m_org;
  // even though the following checks out:
  static_assert(std::copyable<checked_accessor<const double>>);
  static_assert(std::is_assignable_v<checked_accessor<const double>,
                                     checked_accessor<const double>>);
  ...
  static_assert(
      std::is_trivially_assignable_v<MDS, const MDS&> ==
      ((!std::is_class_v<H> ||
        std::is_trivially_assignable_v<
            H, const H&>)&&std::is_trivially_assignable_v<M, const M&> &&
...
constexpr void mixin_layout(const H& handle, const A& acc) {
  ...
  static_assert(std::is_trivially_assignable_v<
                typename std::layout_left::template mapping<std::extents<int>>,
                const typename std::layout_left::template mapping<
                    std::extents<int>>&>);
  ...
  static_assert(!std::is_trivially_assignable_v<
                typename layout_wrapping_integral<4>::template mapping<
                    std::extents<int>>,
                const typename layout_wrapping_integral<4>::template mapping<
                    std::extents<int>>&>);
