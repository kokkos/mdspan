...
//               and acc_ for the values of map_ and acc_ after the invocation
//               of this constructor.
//
...
constexpr void test_mdspan_types(const H&, const M&, const A&) {
  ...
  if constexpr (MDS::rank_dynamic() > 0 && hc && mc && ac) {
    ...
    static_assert(noexcept(MDS()) ==
                  (noexcept(H())&& noexcept(M())&& noexcept(A())));
    assert(m.extents() == typename MDS::extents_type());
    if constexpr (std::equality_comparable<H>) assert(m.data_handle() == H());
    if constexpr (std::equality_comparable<M>) assert(m.mapping() == M());
...
constexpr void mixin_layout(const H& handle, const A& acc) {
  ...
  constexpr size_t D = std::dynamic_extent;
  static_assert(
      !std::is_default_constructible_v<typename layout_wrapping_integral<
          4>::template mapping<std::extents<char, D>>>);
