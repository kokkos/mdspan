...
concept check_mdspan_ctor_implicit =
    requires(MDS m, Args... args) { m = {args...}; };
...
constexpr void test_mdspan_types(const H& handle, const M& map, const A&,
  ...
  if constexpr (mec && ac) {
    ...
    static_assert(
        check_mdspan_ctor_implicit<
            MDS, H, std::array<typename MDS::index_type, MDS::rank_dynamic()>>);
...
constexpr void mixin_extents(const H& handle, const L& layout, const A& acc) {
  ...
  test_mdspan_types<mec, ac>(
      handle, construct_mapping(layout, std::extents<char, D>(7)), acc, 7);
  test_mdspan_types<mec, ac>(
  ...
  test_mdspan_types<mec, ac>(
      ...
      acc, 2, 3);
  test_mdspan_types<mec, ac>(
      handle, construct_mapping(layout, std::extents<char, D, 7, D>(0, 3)), acc,
      0, 3);
  test_mdspan_types<mec, ac>(
      handle,
  ...
  test_mdspan_types<mec, ac>(
      handle, construct_mapping(layout, std::extents<unsigned, 7>()), acc, 7);
  test_mdspan_types<mec, ac>(
      handle, construct_mapping(layout, std::extents<size_t, D, 4, D>(2, 3)),
      acc, 2, 4, 3);
  test_mdspan_types<mec, ac>(
...
constexpr bool test() {
  ...
  static_assert(
      !std::is_constructible_v<mds_t, float*, int, int, std::dextents<int, 1>>);
