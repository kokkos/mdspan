...
constexpr void test_mdspan_ctor_span(
    const H& handle, const M& map, const A&,
    std::span<typename M::index_type, N> exts) {
  ...
  if constexpr (mec && ac) {
    ...
    static_assert(
        !std::is_constructible_v<
            MDS, const H&, std::span<typename MDS::index_type, MDS::rank()>>);
  }
}
...
constexpr bool test() {
  ...
  static_assert(
      !std::is_constructible_v<mds_uchar_t, float*, std::span<IntType, 2>>);
  ...
  static_assert(
      !std::is_constructible_v<mds_int_t, float*, std::span<IntTypeNC, 2>>);
  ...
  static_assert(
      !std::is_constructible_v<mds_t, const float*, std::span<int, 2>>);
