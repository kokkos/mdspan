...
// constexpr mdspan(data_handle_type p, const mapping_type& m, const
// accessor_type& a);
//
...
constexpr void test_mdspan_types(const H& handle, const M& map, const A& acc) {
  ...
  if constexpr (std::equality_comparable<A>) assert(m.accessor() == acc);
}
...
constexpr void mixin_accessor() {
  ...
  static_assert(std::is_default_constructible_v<
                    typename checked_accessor<T>::data_handle_type> !=
...
constexpr bool test() {
  ...
  // sanity check
  static_assert(
      std::is_constructible_v<mds_t, float*,
                              mapping_t<std::extents<int, 3, D, D>>, acc_t>);

  // test non-constructibility from wrong accessor
  static_assert(!std::is_constructible_v<mds_t, float*,
                                         mapping_t<std::extents<int, 3, D, D>>,
  ...
  static_assert(
      !std::is_constructible_v<mds_t, float*,
                               mapping_t<std::extents<int, D, D>>, acc_t>);
  static_assert(
      !std::is_constructible_v<
          mds_t, float*, mapping_t<std::extents<int, D, D, D, D>>, acc_t>);
  // wrong type in general: note the map constructor does NOT convert, since it
  // takes by const&
  static_assert(
      !std::is_constructible_v<mds_t, float*,
                               mapping_t<std::extents<int, D, D, D>>, acc_t>);
  static_assert(
      !std::is_constructible_v<
          mds_t, float*, mapping_t<std::extents<unsigned, 3, D, D>>, acc_t>);

  // test non-constructibility from wrong handle_type
  static_assert(
      !std::is_constructible_v<mds_t, const float*,
                               mapping_t<std::extents<int, 3, D, D>>, acc_t>);
