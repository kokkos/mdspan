...
using lr_mapping_t = typename std::layout_right::template mapping<
...
constexpr void test_no_implicit_conversion() {
  ...
  static_assert(
      std::is_constructible_v<lr_mapping_t<int, D>, ll_mapping_t<int, 5>>);
  static_assert(
      std::is_convertible_v<ll_mapping_t<int, 5>, lr_mapping_t<int, D>>);

  // Check that dynamic to static conversion only works explicitly
  static_assert(
      std::is_constructible_v<lr_mapping_t<int, 5>, ll_mapping_t<int, D>>);
  static_assert(
      !std::is_convertible_v<ll_mapping_t<int, D>, lr_mapping_t<int, 5>>);

  // Sanity check that smaller index_type to larger index_type conversion works
  static_assert(
      std::is_constructible_v<lr_mapping_t<size_t, 5>, ll_mapping_t<int, 5>>);
  static_assert(
      std::is_convertible_v<ll_mapping_t<int, 5>, lr_mapping_t<size_t, 5>>);

  // Check that larger index_type to smaller index_type conversion works
  // explicitly only
  static_assert(
      std::is_constructible_v<lr_mapping_t<int, 5>, ll_mapping_t<size_t, 5>>);
  static_assert(
      !std::is_convertible_v<ll_mapping_t<size_t, 5>, lr_mapping_t<int, 5>>);
}
...
constexpr void test_rank_mismatch() {
  ...
  static_assert(
      !std::is_constructible_v<lr_mapping_t<int, D>, ll_mapping_t<int>>);
  static_assert(
      !std::is_constructible_v<lr_mapping_t<int>, ll_mapping_t<int, D, D>>);
  static_assert(
      !std::is_constructible_v<lr_mapping_t<int, D>, ll_mapping_t<int, D, D>>);
  static_assert(!std::is_constructible_v<lr_mapping_t<int, D, D, D>,
                                         ll_mapping_t<int, D, D>>);
}

constexpr void test_static_extent_mismatch() {
  static_assert(
      !std::is_constructible_v<lr_mapping_t<int, 5>, ll_mapping_t<int, 4>>);
}

constexpr void test_rank_greater_one() {
  constexpr size_t D = std::dynamic_extent;

  static_assert(!std::is_constructible_v<lr_mapping_t<int, D, D>,
                                         ll_mapping_t<int, D, D>>);
  static_assert(!std::is_constructible_v<lr_mapping_t<int, 1, 1>,
                                         ll_mapping_t<int, 1, 1>>);
  static_assert(!std::is_constructible_v<lr_mapping_t<int, D, D, D>,
                                         ll_mapping_t<int, D, D, D>>);
}
