...
void not_extents() {
  // expected-error-re@*:* {{static assertion failed {{.*}}layout_right::mapping
  // template argument must be a specialization of extents}}
  [[maybe_unused]] std::layout_right::mapping<void> mapping;
}

void representable() {
  // expected-error-re@*:* {{static assertion failed {{.*}}layout_right::mapping
  // product of static extents must be representable as index_type.}}
  [[maybe_unused]] std::layout_right::mapping<std::extents<char, 20, 20>>
      mapping;
}
