...
constexpr bool test() {
  ...
  ASSERT_SAME_TYPE(decltype(std::layout_stride::mapping(
                       std::extents<int>(), std::array<unsigned, 0>())),
                   std::layout_stride::template mapping<std::extents<int>>);
  ASSERT_SAME_TYPE(decltype(std::layout_stride::mapping(
                       std::extents<int, 4>(), std::array<char, 1>{1})),
                   std::layout_stride::template mapping<std::extents<int, 4>>);
  ASSERT_SAME_TYPE(decltype(std::layout_stride::mapping(
                       std::extents<int, D>(), std::array<char, 1>{1})),
                   std::layout_stride::template mapping<std::extents<int, D>>);
  ASSERT_SAME_TYPE(
      decltype(std::layout_stride::mapping(std::extents<unsigned, D, 3>(),
                                           std::array<int64_t, 2>{3, 100})),
      std::layout_stride::template mapping<std::extents<unsigned, D, 3>>);

  ASSERT_SAME_TYPE(decltype(std::layout_stride::mapping(
                       std::extents<int>(), std::span<unsigned, 0>())),
                   std::layout_stride::template mapping<std::extents<int>>);
  ASSERT_SAME_TYPE(
      decltype(std::layout_stride::mapping(std::extents<int, 4>(),
                                           std::declval<std::span<char, 1>>())),
      std::layout_stride::template mapping<std::extents<int, 4>>);
  ASSERT_SAME_TYPE(
      decltype(std::layout_stride::mapping(std::extents<int, D>(),
                                           std::declval<std::span<char, 1>>())),
      std::layout_stride::template mapping<std::extents<int, D>>);
  ASSERT_SAME_TYPE(
      decltype(std::layout_stride::mapping(
          std::extents<unsigned, D, 3>(),
          std::declval<std::span<int64_t, 2>>())),
      std::layout_stride::template mapping<std::extents<unsigned, D, 3>>);
  return true;
}
