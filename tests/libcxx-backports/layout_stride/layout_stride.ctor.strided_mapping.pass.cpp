...
//   - is_constructible_v<extents_type, typename
//   StridedLayoutMapping::extents_type> is true.
...
//
// Effects: Direct-non-list-initializes extents_ with other.extents(), and for
...
constexpr void test_conversion() {
  ...
  constexpr bool l_convertible = std::is_same_v<FromL, std::layout_right> ||
                                 std::is_same_v<FromL, std::layout_left> ||
...
using ToM = typename std::layout_stride::template mapping<
...
constexpr void test_no_implicit_conversion() {
  ...
  // Sanity check that one static to dynamic conversion works
  static_assert(
      std::is_constructible_v<ToM<int, D, 7>, FromM<FromL, int, 5, 7>>);
  static_assert(std::is_convertible_v<FromM<FromL, int, 5, 7>, ToM<int, D, 7>>);

  // Check that dynamic to static conversion only works explicitly
  static_assert(
      std::is_constructible_v<ToM<int, 5, 7>, FromM<FromL, int, D, 7>>);
  static_assert(
      !std::is_convertible_v<FromM<FromL, int, D, 7>, ToM<int, 5, 7>>);
...
constexpr void test_rank_mismatch() {
  ...
  static_assert(
      !std::is_constructible_v<ToM<int, D, D, D>, FromM<FromL, int, D, D>>);
}
...
constexpr void test_static_extent_mismatch() {
  ...
  static_assert(
      !std::is_constructible_v<ToM<int, D, 5>, FromM<FromL, int, D, 4>>);
  static_assert(!std::is_constructible_v<ToM<int, 5>, FromM<FromL, int, 4>>);
  static_assert(
      !std::is_constructible_v<ToM<int, 5, D>, FromM<FromL, int, 4, D>>);
}
