...
//     constexpr explicit(N != rank_dynamic()) extents(span<OtherIndexType, N>
//     exts) noexcept;
//
// Constraints:
...
int main(int, char**) {
  ...
  static_assert(!std::is_constructible_v<E, std::span<int, 1>>,
  ...
  static_assert(!std::is_constructible_v<E, std::span<int, 5>>,
                "extents constructible from illegal arguments");
  ...
  static_assert(
      std::is_constructible_v<std::extents<int, D, D, 5, D, D>, decltype(s5)>,
      "extents unexpectectly not constructible");
  // check that implicit construction doesn't work
  assert((implicit_construction<std::extents<int, D, D, 5, D, D>>(s5).value ==
          false));
  ...
  static_assert(
      !std::is_constructible_v<std::dextents<int, 2>, std::span<IntTypeNC, 2>>);
  return 0;
}
