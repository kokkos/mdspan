...
// exts_arr be array<index_type,
// N>{static_cast<index_type>(std::move(exts))...}.
//
// Constraints:
...
//
// Preconditions:
//   * If N != rank_dynamic() is true, exts_arr[r] equals Er for each r for
//   which
//     Er is a static extent, and
//   * either
//     - sizeof...(exts) == 0 is true, or
//     - each element of exts is nonnegative and is representable as a value of
//     type index_type.
//
...
struct IntegralCtorTest {
  ...
  static constexpr void test_construction(AllExtents all_ext, Extents ext,
...
int main(int, char**) {
  ...
  static_assert(!std::is_constructible_v<E, int>,
  ...
  static_assert(!std::is_constructible_v<E, int, int, int, int, int>,
                "extents constructible from illegal arguments");
  ...
  static_assert(
      !std::is_constructible_v<std::extents<unsigned long, D>, IntType>,
      "extents constructible from illegal arguments");
  return 0;
}
