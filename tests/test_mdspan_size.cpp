...
void test_mdspan_size(std::vector<char>& storage, Extents&& e) {
  ...
  const size_type min_storage_size =
      static_cast<size_type>(product_of_extents(e));
...
TEST(TestMdspan, MdspanSizeReturnTypeAndPrecondition) {
  ...
  static_assert(std::numeric_limits<std::int8_t>::max() == 127,
                "max int8_t != 127");
  test_mdspan_size(storage,
                   Kokkos::extents<std::int8_t, 12, 11>{});  // 12 * 11 == 132
}
