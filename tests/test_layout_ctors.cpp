...
struct TestLayoutCompatCtors<
    std::tuple<Mapping, std::integer_sequence<size_t, DynamicSizes...>,
               Mapping2, std::integer_sequence<size_t, DynamicSizes2...>>>
    : public ::testing::Test {
...
using test_left_type_compatible =
    std::tuple<typename Kokkos::layout_left::template mapping<E1>, S1,
               typename Kokkos::layout_left::template mapping<E2>, S2>;
template <class E1, class S1, class E2, class S2>
using test_right_type_compatible =
    std::tuple<typename Kokkos::layout_right::template mapping<E1>, S1,
...
using compatible_layout_test_types = ::testing::Types<
    ...
    //--------------------
    test_case_type<exts<dyn, dyn, 15>, sizes<5, 10>, exts<5, dyn, 15>,
                   sizes<10>>,
    test_case_type<exts<5, 10, 15>, sizes<>, exts<5, dyn, 15>, sizes<10>>,
    test_case_type<exts<5, 10, 15>, sizes<>, exts<dyn, dyn, dyn>,
                   sizes<5, 10, 15>>>;

using left_compatible_test_types =
    compatible_layout_test_types<test_left_type_compatible>;
using right_compatible_test_types =
    compatible_layout_test_types<test_right_type_compatible>;
...
TYPED_TEST(TestLayoutLeftCompatCtors, compatible_construct_1) {
  // The compatible mapping constructor ensures extents() == other.extents() is
  // true.
...
TEST(TestLayoutLeftListInitialization, test_layout_left_extent_initialization) {
  Kokkos::layout_left::mapping<Kokkos::extents<size_t, dyn, dyn>> m{
      Kokkos::dextents<size_t, 2>{16, 32}};
...
TEST(TestLayoutRightListInitialization,
     test_layout_right_extent_initialization) {
  Kokkos::layout_right::mapping<Kokkos::extents<size_t, dyn, dyn>> m{
      Kokkos::dextents<size_t, 2>{16, 32}};
  ASSERT_EQ(m.extents().rank(), 2);
  ASSERT_EQ(m.extents().rank_dynamic(), 2);
  ASSERT_EQ(m.extents().extent(0), 16);
