...
TEST(TestMdspanListInitializationLayoutLeft,
     test_mdspan_list_initialization_layout_left) {
  std::array<int, 1> d{42};
  Kokkos::mdspan<int, Kokkos::extents<size_t, dyn, dyn>, Kokkos::layout_left> m{
      d.data(), 16, 32};
  ASSERT_EQ(m.data_handle(), d.data());
  ASSERT_EQ(m.rank(), 2);
  ASSERT_EQ(m.rank_dynamic(), 2);
...
TEST(TestMdspanListInitializationLayoutRight,
     test_mdspan_list_initialization_layout_right) {
  std::array<int, 1> d{42};
  Kokkos::mdspan<int, Kokkos::extents<size_t, dyn, dyn>, Kokkos::layout_right>
      m{d.data(), 16, 32};
  ASSERT_EQ(m.data_handle(), d.data());
  ASSERT_EQ(m.rank(), 2);
  ASSERT_EQ(m.rank_dynamic(), 2);
...
TEST(TestMdspanListInitializationLayoutStride,
     test_mdspan_list_initialization_layout_stride) {
  std::array<int, 1> d{42};
  Kokkos::mdspan<int, Kokkos::extents<size_t, dyn, dyn>, Kokkos::layout_stride>
      m{d.data(),
        {Kokkos::dextents<size_t, 2>{16, 32},
         std::array<std::size_t, 2>{1, 128}}};
  ASSERT_EQ(m.data_handle(), d.data());
  ASSERT_EQ(m.rank(), 2);
  ASSERT_EQ(m.rank_dynamic(), 2);
  ASSERT_EQ(m.extent(0), 16);
...
TEST(TestMdspanCTAD, ctad_const_carray) {
  ...
  Kokkos::mdspan m(data);
  static_assert(
      std::is_same<typename decltype(m)::element_type, const int>::value);
  ASSERT_EQ(m.data_handle(), &data[0]);
  ASSERT_EQ(m.rank(), 1);
  ASSERT_EQ(m.rank_dynamic(), 0);
  ASSERT_EQ(m.static_extent(0), 5);
...
TEST(TestMdspanCTAD, layout_left) {
  ...
  Kokkos::mdspan m0{d.data(),
                    Kokkos::layout_left::mapping{Kokkos::extents{16, 32}}};
...
TEST(TestMdspanCTAD, layout_right) {
  ...
  Kokkos::mdspan m0{d.data(),
                    Kokkos::layout_right::mapping{Kokkos::extents{16, 32}}};
  ASSERT_EQ(m0.data_handle(), d.data());
  ASSERT_EQ(m0.rank(), 2);
  ASSERT_EQ(m0.rank_dynamic(), 2);
  ASSERT_EQ(m0.extent(0), 16);
...
TEST(TestMdspanCTAD, layout_stride) {
  ...
  Kokkos::mdspan m0{d.data(), Kokkos::layout_stride::mapping{
                                  Kokkos::extents{16, 32}, std::array{1, 128}}};
  ASSERT_EQ(m0.data_handle(), d.data());
  ASSERT_EQ(m0.rank(), 2);
  ASSERT_EQ(m0.rank_dynamic(), 2);
  ASSERT_EQ(m0.extent(0), 16);
  ...
  /*
  Kokkos::mdspan m1{d.data(), Kokkos::layout_stride::mapping{Kokkos::extents{16,
  32}, Kokkos::extents{1, 128}}}; ASSERT_EQ(m1.data(), d.data());
  ASSERT_EQ(m1.rank(), 2);
  ASSERT_EQ(m1.rank_dynamic(), 2);
  ASSERT_EQ(m1.extent(0), 16);
  ASSERT_EQ(m1.extent(1), 32);
  ...
  */
  ...
  // TODO: Perhaps one day I'll get this to work.
  /*
    Kokkos::mdspan m2{d.data(), Kokkos::layout_stride::mapping{{16, 32}, {1,
    128}}}; ASSERT_EQ(m2.data_handle(), d.data()); ASSERT_EQ(m2.rank(), 2);
